...
class _CrearEventoScreenState extends State<CrearEventoScreen> {
  ...
  Future<void> _pickAndUploadEventImage() async {
    ...
    try {
      ...
      final targetPath = "${tempDir.path}/event_${DateTime.now().millisecondsSinceEpoch}.jpg";

      final XFile? compressedFile = await FlutterImageCompress.compressAndGetFile(
        pickedFile.path, targetPath, quality: 60, minWidth: 800, minHeight: 600,
      );
  ...
  void _abrirMapaCompleto() async {
    final selectedVenue = await Navigator.push(
      ...
      MaterialPageRoute(
        builder: (context) => MapaPantallaCompletaEventoScreen(
          venues: _venues, selectedSport: _selectedSport, selectedZone: _selectedZone,
        ),
      ),
    );
    ...
    if (selectedVenue != null) {
      ...
      mapboxMap?.flyTo(
          CameraOptions(center: Point(coordinates: Position(selectedVenue['lng'] as num, selectedVenue['lat'] as num)), zoom: 14.5, pitch: 45.0),
  ...
  Future<void> _crearEvento() async {
    ...
    try {
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Evento publicado exitosamente'), backgroundColor: Colors.green));
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Crear Evento', style: TextStyle(color: Color(0xFF111827), fontWeight: FontWeight.w700)),
        iconTheme: const IconThemeData(color: Color(0xFF111827)),
      ),
      body: _isLoadingVenues
          ? const Center(child: CircularProgressIndicator(color: Color(0xFF155DFC)))
                ...
                children: [
                    ...
                    children: [
                      ...
                      if (_selectedLocation != null)
                        Container(
                          ...
                          decoration: BoxDecoration(color: const Color(0xFF155DFC).withValues(alpha: 0.1), borderRadius: BorderRadius.circular(8)),
                          child: Text(_selectedLocation!, style: const TextStyle(color: Color(0xFF155DFC), fontSize: 12, fontWeight: FontWeight.bold)),
                        ),
                    ],
                  ),
                  const SizedBox(height: 12),
                      ...
                      children: ['Norte', 'Sur', 'Centro', 'Oriente', 'Occidente'].map((zone) {
                        ...
                        return Padding(
                          ...
                          child: ChoiceChip(
                            ...
                            selected: isSelected, selectedColor: const Color(0xFF155DFC), backgroundColor: Colors.white,
                            onSelected: (selected) {
                    ...
                    child: Container(
                      ...
                      child: Stack(
                        children: [
                          AbsorbPointer(
                            child: MapWidget(
                              key: const ValueKey("mapboxMapEvent_small"), textureView: true, styleUri: MapboxStyles.MAPBOX_STREETS,
                          ...
                          Positioned(
                            ...
                            child: Container(
                              ...
                              child: const Row(children: [Icon(Icons.search, color: Color(0xFF155DFC)), SizedBox(width: 10), Text('Toca para buscar en pantalla completa...', style: TextStyle(color: Colors.grey, fontSize: 13))]),
                            ),
                          ),
                          Positioned(
                            bottom: 10, right: 10,
                            child: FloatingActionButton.small(heroTag: "expandBtnEvent", backgroundColor: Colors.black87, onPressed: _abrirMapaCompleto, child: const Icon(Icons.fullscreen, color: Colors.white)),
                          )
                        ],
                      ),
                    ),
                  ...
                  SizedBox(
                    ...
                    child: SegmentedButton<String>(
                      segments: const [
                        ButtonSegment<String>(value: 'upcoming', label: Text('Próximo', style: TextStyle(fontSize: 14))),
                        ButtonSegment<String>(value: 'ongoing', label: Text('En Curso', style: TextStyle(fontSize: 14))),
                      ],
                      selected: {_status},
                      onSelectionChanged: (Set<String> newSelection) => setState(() => _status = newSelection.first),
                      style: ButtonStyle(backgroundColor: WidgetStateProperty.resolveWith<Color>((Set<WidgetState> states) => states.contains(WidgetState.selected) ? const Color(0xFF155DFC).withValues(alpha: 0.15) : Colors.white)),
                    ),
                  ),
                  ...
                  Container(
                    ...
                    child: SwitchListTile(
                      ...
                      subtitle: Text(widget.isAdmin ? "Aparecerá gigante en la pantalla." : "Solo los administradores pueden destacar."),
                      value: _isFeatured,
                      activeThumbColor: const Color(0xFFFF6900), activeTrackColor: const Color(0xFFFF6900).withValues(alpha: 0.3),
                      secondary: Icon(Icons.star, color: widget.isAdmin ? const Color(0xFFFF6900) : Colors.grey),
                      onChanged: widget.isAdmin ? (bool value) => setState(() => _isFeatured = value) : null,
                    ),
                  ),
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: Colors.white, borderRadius: BorderRadius.circular(20), border: Border.all(color: const Color(0xFFE5E7EB), width: 2),
          image: _eventImageUrl != null ? DecorationImage(image: NetworkImage(_eventImageUrl!), fit: BoxFit.cover) : null,
        ),
        child: _isUploadingImage
            ...
            : _eventImageUrl == null
                ? Column(
                    ...
                    children: [Icon(Icons.add_photo_alternate_rounded, size: 50, color: Colors.grey.shade400), const SizedBox(height: 10), Text("Añadir foto del evento (Opcional)", style: TextStyle(color: Colors.grey.shade500, fontWeight: FontWeight.w600))],
                  )
                : Align(alignment: Alignment.bottomRight, child: Padding(padding: const EdgeInsets.all(12.0), child: Container(padding: const EdgeInsets.all(8), decoration: const BoxDecoration(color: Colors.black54, shape: BoxShape.circle), child: const Icon(Icons.edit, color: Colors.white, size: 20)))),
      ),
    );
  }
    ...
    return Container(
      ...
      child: SizedBox(
        ...
        child: ElevatedButton(
          style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF155DFC), disabledBackgroundColor: Colors.grey.shade300, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(14))),
          onPressed: (_isLoading || !_isFormValid) ? null : _crearEvento,
          child: _isLoading ? const CircularProgressIndicator(color: Colors.white) : Text(_isFormValid ? 'PUBLICAR EVENTO' : 'COMPLETE TODOS LOS CAMPOS', style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w800, color: Colors.white)),
        ),
      ),
    );
  }
  ...
  Widget _input(TextEditingController controller, String hint) => Container(
        ...
        child: TextField(controller: controller, onChanged: (_) => setState(() {}), decoration: InputDecoration(border: InputBorder.none, hintText: hint)),
      );
  ...
  Widget _circleBtn(IconData icon, VoidCallback onTap) => GestureDetector(onTap: onTap, child: Container(width: 40, height: 40, decoration: const BoxDecoration(color: Color(0xFF155DFC), shape: BoxShape.circle), child: Icon(icon, color: Colors.white, size: 20)));
  ...
  Widget _priceBox() => Container(
        ...
        child: TextField(
          ...
          style: const TextStyle(fontSize: 28, color: Color(0xFF155DFC), fontWeight: FontWeight.bold),
...
class MapaPantallaCompletaEventoScreen extends StatefulWidget {
  ...
  const MapaPantallaCompletaEventoScreen({super.key, required this.venues, this.selectedSport, this.selectedZone});

  @override
  State<MapaPantallaCompletaEventoScreen> createState() => _MapaPantallaCompletaEventoScreenState();
}

class _MapaPantallaCompletaEventoScreenState extends State<MapaPantallaCompletaEventoScreen> {
  MapboxMap? mapboxMap;
  PointAnnotationManager? pointAnnotationManager;
  String _searchQuery = "";
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: Stack(
        children: [
          MapWidget(key: const ValueKey("mapboxMapEvent_fullscreen"), textureView: true, styleUri: MapboxStyles.MAPBOX_STREETS, onMapCreated: _onMapCreated),
          SafeArea(
            child: Padding(
              padding: const EdgeInsets.all(16.0),
              child: Row(
                children: [
                  FloatingActionButton(heroTag: "backBtnEvent", mini: true, backgroundColor: Colors.white, child: const Icon(Icons.arrow_back, color: Colors.black), onPressed: () => Navigator.pop(context)),
                  ...
                  Expanded(
                    child: Container(
                      ...
                      child: TextField(
                        ...
                        decoration: const InputDecoration(icon: Icon(Icons.search, color: Color(0xFF155DFC)), hintText: 'Buscar lugar por nombre...', border: InputBorder.none),
                      ),
                    ),
                  ),
                ],
